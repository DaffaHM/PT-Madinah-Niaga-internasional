...
export default function Header() {
  ...
  // Animasi buka/tutup menu — dijalankan setiap kali isOpen berubah
  useEffect(() => {
    if (!menuPanelRef.current || !backdropRef.current) return

    if (isOpen) {
      // Reset posisi dulu
      gsap.set(menuPanelRef.current, { y: -12, opacity: 0, display: 'block' })
      gsap.set(backdropRef.current, { opacity: 0, display: 'block' })

      // Animate masuk
      gsap.to(backdropRef.current, { opacity: 1, duration: 0.2, ease: 'none' })
      gsap.to(menuPanelRef.current, { y: 0, opacity: 1, duration: 0.25, ease: 'power2.out' })
      ...
      gsap.to(menuPanelRef.current, {
        ...
        onComplete: () => gsap.set(menuPanelRef.current, { display: 'none' })
      })
    }
  }, [isOpen])
  ...
  return (
    <>
      ...
      {/* Mobile Menu — selalu ada di DOM, visibility dikontrol GSAP */}
      <div className="fixed inset-0 top-20 z-40 md:hidden pointer-events-none">
        {/* Backdrop */}
        <div
          ref={backdropRef}
        ...
        <div
          ref={menuPanelRef}
          className="relative bg-white shadow-lg pointer-events-auto"
          style={{ display: 'none', opacity: 0 }}
        >
